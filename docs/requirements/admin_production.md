...
| mask_coverage | 遮罩佔圖片面積比例 0~1（純 sam_points 等 worker 推論時為 null）|
...
**範圍**：A 與 B 都是**區域層級（region-level）**，不是色塊層級。
admin 點 template.svg 上某一格時，前端取該 polygon 的 SVG `id="rN"` 屬性，把 `polygon_id`
傳給後端。後端不分析座標、不做連通區域分析，直接用 SVG 內標好的 id 操作。

**操作 A：格子合併**

- 點選 template.svg 上某一格 → 前端拿該 polygon 的 `id="r5"`
- admin 從調色盤下拉選擇目標色號 (`target_template_id=2`)
- 後端：解 SVG 取 `polygon[id="r5"]` 頂點 → cv2.fillPoly 畫 mask → 把 mask 內 snapped_rgb
  像素改成 target_template_id 對應的 RGB → 重跑 `output_to_svg` + `output_filled_from_template`
- 若該 polygon 是「該色」最後一格，被合併色號自動從 `palette_json` 中移除
  （`output_to_svg` 從 `getUniqueColorsMasks()` 推 unique colors，0 像素的色不會出現）
- `palette_color_mappings`：用 `algorithm_rgb` 在新 palette 中重新匹配，找不到的 row（即被
  合併沒了的色號）DELETE，找到的 UPDATE template_id（output_to_svg 會依面積重編號）
- `production_job.approved` 退回 `false`，需重新進入顏色對應確認流程

**操作 B：消除邊界線**

- 點選 template.svg 上兩個相鄰格子（前端兩 click + 對話框問哪邊保留）
- admin 確認 → 前端送 `{absorbed_polygon_id: "r5", surviving_polygon_id: "r2"}`
- 後端：解 SVG 取 absorbed polygon 頂點 → mask → 把 mask 內像素改成 surviving 那格在
  snapped_rgb 中的實際 RGB（從 SVG 採樣，不從 palette 反查）→ 重跑 SVG + filled
- 鄰接判定為前端 UI 約束，後端不驗證；若 admin 送了不相鄰的兩格，後端仍會執行（實際結果
  是改 absorbed 那格的色）
- palette / mappings 縮減邏輯與 A 同
- `production_job.approved` 退回 `false`

> 註：原規劃的「操作 C：輪廓平滑」已下架。理由：相鄰 polygon 共享邊界的 Chaikin
> 對齊實作工時不划算，且現行 output_to_svg 內建的 _smooth_quantized 已提供基本平滑。
...
3. 透過 SVG → PDF 引擎轉換（**svglib → cairosvg 兩層 fallback**，沿用 print_batch 模組的 `_render_svg_with_fallbacks`）：
   - **Layer 1 svglib**（向量保留、檔案最小）：將 ReportLab `Drawing` 縮放到目標 cm × cm 後 `renderPDF.draw`
   - **Layer 2 cairosvg**（svglib 解析失敗時降級）：rasterize 為 PNG @ 300 DPI 後 `drawImage` 嵌入
   - 兩者皆失敗 → 回 400「SVG 渲染失敗」（不走佔位框，避免出貨低品質檔案）
4. 直接回傳 PDF 下載（`Content-Disposition: attachment`），不上傳至 Firebase
5. 若 SVG 有更新，每次匯出都重新轉換，確保與最新 SVG 一致

> **不走 Inkscape**：原規格曾考慮以 Inkscape 做「文字轉路徑」確保印刷廠端字型一致，但 Inkscape 是 Linux 重型依賴（Docker image 增重 ≈ 200MB、單次轉換 5–15s）。
> 取捨：SVG 在生成端（PBN 引擎）就應該已經是純 polygon，不含 `<text>`；若未來引入文字標註，需在引擎端先 path-ify 再寫入 SVG，而不是出 PDF 時再做。
> 此契約由 print_batch 與 production 模組共用同一份 `_render_svg_with_fallbacks` 落實。
...
| mask_coverage | 遮罩佔圖片面積比例 0~1（nullable，純 sam_points 等 worker 推論時為 null）|
