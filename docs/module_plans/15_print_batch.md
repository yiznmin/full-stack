...
### B. PDF 生成失敗
網路抓 SVG 失敗、SVG 解析失敗 → 整 batch finalize 失敗（rollback 或留 draft）。  
**預設**：rollback，狀態仍 draft，報錯訊息給 admin。

### C. requirements.txt 新增依賴
- `svglib` (Apache 2.0)
- `reportlab` (BSD)
- `Pillow` (HPND)
- `cairosvg` (LGPL — fallback only)
- `httpx` (已有)

要不要 cairosvg？依賴 cairo C 庫，安裝較重；svglib 通常夠用。

**預設**：先用 svglib + reportlab + Pillow，測試時若 SVG 解析失敗再加 cairosvg。
